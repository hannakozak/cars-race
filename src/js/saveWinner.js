import { addWinner, state, updateWinner } from "../services/winners";
...
export const saveWinner = async () => {
  ...
  const savedWinner = savedWinners.find(
    (winner) => winner.id === currentWinnerId
  );
  const dataToAdd = {
    id: +state.results[0].id,
    wins: 1,
    time: state.results[0].time,
  };
  let numberOfWins = 1;
  if (savedWinner) {
    ...
    await updateWinner(currentWinnerId, {
      wins: numberOfWins,
